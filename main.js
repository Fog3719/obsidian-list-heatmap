...
var FileParser = class {
  ...
  getDiaryFiles(folderPath) {
    return __async(this, null, function* () {
      ...
      const files = folder.children.filter((file) => file instanceof import_obsidian.TFile && file.extension === "md").map((file) => file).filter((file) => /^\d{4}-\d{2}-\d{2}\.md$/.test(file.name));
      return files;
    });
  }
  parseFile(file) {
    ...
    return __async(this, null, function* () {
      ...
      for (const file of files) {
        try {
          ...
          const content = yield this.fileParser.parseFile(file);
...
var DataCache = class {
  constructor(plugin) {
    this.cacheData = null;
    this.CACHE_FILE = "list-heatmap-cache.json";
  ...
  updateCache(listCounts, settings) {
    return __async(this, null, function* () {
      ...
      this.cacheData = {
        ...
        data: __spreadValues({}, listCounts)
      };
  ...
  getCachedData(settings) {
    return __async(this, null, function* () {
      ...
      if (!this.cacheData) {
        this.cacheData = yield this.loadCache();
      }
      if (!this.cacheData || this.cacheData.version !== this.CACHE_VERSION) {
        return null;
      }
      if (this.cacheData.settings.diaryFolderPath !== settings.diaryFolderPath || JSON.stringify(this.cacheData.settings.customTitles) !== JSON.stringify(settings.customTitles)) {
        return null;
      }
...
var HeatmapView = class extends import_obsidian2.ItemView {
  ...
  createControls() {
    ...
    const currentTime = timeNav.createEl("span", {
      text: this.currentView === "year" ? `${this.currentYear}` : `${this.currentYear} ${this.currentMonth}`
    });
  ...
  renderYearHeatmap(data) {
    ...
    svg.selectAll(".day").data(yearData).enter().append("rect").attr("class", "day").attr("width", cellSize).attr("height", cellSize).attr("x", (d) => (cellSize + cellMargin) * d.week).attr("y", (d) => (cellSize + cellMargin) * d.day).attr("fill", (d) => this.getColorForCount(d.count)).attr("rx", 2).attr("ry", 2).append("title").text((d) => `${d.date}: ${d.count} items`);
  }
  renderMonthHeatmap(data) {
    ...
    svg.selectAll(".day").data(monthData).enter().append("g").attr("class", "day-cell").attr("transform", (d) => {
      ...
      select_default2(this).append("rect").attr("width", cellSize).attr("height", cellSize).attr("fill", d.count > 0 ? view.getColorForCount(d.count) : "#f0f0f0").attr("rx", 4).attr("ry", 4);
      select_default2(this).append("text").attr("x", cellSize / 2).attr("y", cellSize / 3).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", d.count > 10 ? "#fff" : "#333").text(d.day);
      if (d.count > 0) {
        select_default2(this).append("text").attr("x", cellSize / 2).attr("y", cellSize * 2 / 3).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", d.count > 10 ? "#fff" : "#333").attr("font-size", "0.8em").text(d.count);
      }
    });
  }
...
var ListHeatmapSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Diary folder path").setDesc("Specify the folder path containing diary files").addText((text) => text.setPlaceholder("\u4F8B\u5982: \u65E5\u8BB0/").setValue(this.plugin.settings.diaryFolderPath).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian3.Setting(containerEl).setName("Count headings").setDesc("Specify headings to count unordered lists under (comma separated)").addText((text) => text.setPlaceholder("\u4F8B\u5982: \u4ECA\u65E5\u4EFB\u52A1, \u5F85\u529E\u4E8B\u9879").setValue(this.plugin.settings.customTitles.join(", ")).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian3.Setting(containerEl).setName("Heatmap color settings").setDesc("Configure heatmap color ranges (format: 1-5:#FF6B6B,6-10:#FF8E8E,...)").addText((text) => text.setPlaceholder("\u4F8B\u5982: 1-5:#FF6B6B,6-10:#FF8E8E,11-15:#FFA5A5,16-20:#FFC7C7,21+:#FFE8E8").setValue(this.plugin.settings.colorRanges.map((range) => `${range.min}-${range.max === Number.MAX_SAFE_INTEGER ? "+" : range.max}:${range.color}`).join(",")).onChange((value) => __async(this, null, function* () {
      try {
        const ranges = value.split(",").map((range) => {
          const [rangeStr, color2] = range.split(":");
          ...
          return { min, max, color: color2.trim() };
        });
    ...
    new import_obsidian3.Setting(containerEl).setName("Default view").setDesc("Set default time range for heatmap display").addDropdown((dropdown) => dropdown.addOption("year", "Year view").addOption("month", "Month view").setValue(this.plugin.settings.defaultView).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.defaultView = value;
      yield this.plugin.saveSettings();
      this.plugin.refreshView();
    })));
    new import_obsidian3.Setting(containerEl).setName("Enable cache").setDesc("Enable data caching for better performance").addToggle((toggle) => toggle.setValue(this.plugin.settings.cacheEnabled).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian3.Setting(containerEl).setName("Refresh data").setDesc("Manually refresh list statistics").addButton((button) => button.setButtonText("Refresh").onClick(() => __async(this, null, function* () {
      yield this.plugin.refreshData();
    })));
  }
};
