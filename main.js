...
var FileParser = class {
  ...
  getDiaryFiles(folderPath) {
    return __async(this, null, function* () {
      const folder = this.app.vault.getAbstractFileByPath(folderPath);
      if (!folder || !(folder instanceof import_obsidian.TFolder)) {
        return [];
      }
      const diaryFiles = folder.children.filter((file) => file instanceof import_obsidian.TFile && file.extension === "md" && this.isDiaryFilename(file.basename));
      return diaryFiles;
    });
  }
  isDiaryFilename(filename) {
...
var ListCounter = class {
  ...
  countLists(files, titles) {
    return __async(this, null, function* () {
      ...
      for (const file of files) {
        ...
        const content = yield this.fileParser.readFileContent(file);
  ...
  countListsInContent(content, titles) {
    let totalCount = 0;
    for (const title of titles) {
      ...
      if (titleMatch && titleMatch.index !== void 0) {
        ...
        const nextTitlePos = nextTitleMatch && nextTitleMatch.index !== void 0 ? titlePos + 1 + nextTitleMatch.index : content.length;
...
var DataCache = class {
  ...
  updateCache(data, settings) {
    return __async(this, null, function* () {
      ...
      yield this.plugin.app.vault.adapter.write(cacheFilePath, JSON.stringify(cache, null, 2));
  ...
  getCachedData(settings) {
    return __async(this, null, function* () {
      try {
        const cacheFilePath = ".obsidian-list-heatmap-cache.json";
        const cacheContent = yield this.plugin.app.vault.adapter.read(cacheFilePath);
        const cache = JSON.parse(cacheContent);
        if (!cache || !cache.cacheData) {
          return null;
        }
        if (cache.cacheSettings.diaryFolderPath !== settings.diaryFolderPath || JSON.stringify(cache.cacheSettings.customTitles) !== JSON.stringify(settings.customTitles)) {
          return null;
        }
        this.lastUpdated = cache.cacheLastUpdated;
  ...
  clearCache() {
    return __async(this, null, function* () {
      try {
        const cacheFilePath = ".obsidian-list-heatmap-cache.json";
        yield this.plugin.app.vault.adapter.write(cacheFilePath, JSON.stringify({}, null, 2));
        this.lastUpdated = null;
      } catch (error) {
...
// src/heatmapView.ts
var VIEW_TYPE_HEATMAP = "list-heatmap-view";
var HeatmapView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.plugin = plugin;
    this.currentView = this.plugin.settings.defaultView;
    const now2 = new Date();
    this.currentYear = now2.getFullYear();
    this.currentMonth = now2.getMonth() + 1;
  }
  getViewType() {
  ...
  createControls() {
    ...
    const currentTime = timeNav.createEl("span", {
      text: this.currentView === "year" ? `${this.currentYear}` : `${this.currentYear}-${this.currentMonth.toString().padStart(2, "0")}`
    });
  ...
  renderYearHeatmap(data) {
    ...
    svg.selectAll(".day").data(yearData).enter().append("rect").attr("class", "day").attr("width", cellSize).attr("height", cellSize).attr("x", (d) => (cellSize + cellMargin) * (d.week || 0)).attr("y", (d) => (cellSize + cellMargin) * d.day).attr("fill", (d) => this.getColorForCount(d.count)).attr("rx", 2).attr("ry", 2).attr("cursor", (d) => d.filename ? "pointer" : "default").on("click", function(event, d) {
      if (d.filename) {
...
File: ${d.filename}`;
  ...
  renderMonthHeatmap(data) {
    ...
    const svg = select_default2(this.heatmapContainer).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", `translate(${margin.left},${margin.top})`);
    const weekdays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    svg.selectAll(".weekday-label").data(weekdays).enter().append("text").attr("class", "weekday-label").attr("x", (d, i) => (cellSize + cellMargin) * i + cellSize / 2).attr("y", -15).attr("text-anchor", "middle").text((d) => d);
    const getColorForCount = this.getColorForCount.bind(this);
    const plugin = this.plugin;
    svg.selectAll(".day").data(monthData).enter().append("g").attr("class", "day-cell").attr("transform", (d) => {
    ...
    }).attr("cursor", (d) => d.filename ? "pointer" : "default").on("click", function(event, d) {
      if (d.filename) {
        plugin.openDiaryFile(d.filename);
      }
    }).each(function(d) {
      const self = select_default2(this);
      self.append("rect").attr("width", cellSize).attr("height", cellSize).attr("fill", d.count > 0 ? getColorForCount(d.count) : "#f0f0f0").attr("rx", 4).attr("ry", 4);
      self.append("text").attr("x", cellSize / 2).attr("y", cellSize / 3).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", d.count > 10 ? "#fff" : "#333").text(d.day);
      if (d.count > 0) {
        self.append("text").attr("x", cellSize / 2).attr("y", cellSize * 2 / 3).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", d.count > 10 ? "#fff" : "#333").attr("font-size", "0.8em").text(d.count);
      }
      if (d.filename) {
        self.append("title").text(`${d.date}: ${d.count} list items
File: ${d.filename}`);
      } else {
        self.append("title").text(`${d.date}: ${d.count} list items`);
      }
  ...
  prepareYearData(data, year) {
    ...
    for (let d = new Date(yearStart); d <= yearEnd; d.setDate(d.getDate() + 1)) {
      ...
      const day = d.getDay();
      const dayOfYear = Math.floor((d.getTime() - yearStart.getTime()) / (24 * 60 * 60 * 1e3));
  ...
  prepareMonthData(data, year, month) {
    const result = [];
  ...
  getMonthPositions(year) {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const dayOfYear = Math.floor((firstDay.getTime() - new Date(year, 0, 1).getTime()) / (24 * 60 * 60 * 1e3));
      const week = Math.floor(dayOfYear / 7);
...
// src/main.ts
var ListHeatmapPlugin = class extends import_obsidian3.Plugin {
  onload() {
    return __async(this, null, function* () {
  ...
  refreshData() {
    return __async(this, null, function* () {
      const diaryFiles = yield this.fileParser.getDiaryFiles(this.settings.diaryFolderPath);
      const listCounts = yield this.listCounter.countLists(diaryFiles, this.settings.customTitles);
      yield this.dataCache.updateCache(listCounts, this.settings);
      this.refreshView();
    });
  }
  activateView() {
  ...
  getDiaryFilename(date) {
    ...
    const file = folder.children.find((file2) => file2 instanceof import_obsidian3.TFile && file2.name === possibleFilename);
    return file ? file.path : void 0;
  }
  openDiaryFile(filePath) {
...
var ListHeatmapSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Diary Folder Path").setDesc("Specify the folder path containing diary files").addText((text) => text.setPlaceholder("Example: Diary/").setValue(this.plugin.settings.diaryFolderPath).onChange((value) => __async(this, null, function* () {
      if (value !== this.plugin.settings.diaryFolderPath) {
        yield this.plugin.dataCache.clearCache();
      }
      this.plugin.settings.diaryFolderPath = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian3.Setting(containerEl).setName("Count Titles").setDesc("Specify titles under which to count unordered lists (separate multiple titles with commas)").addText((text) => text.setPlaceholder("Example: Daily Tasks, To-Do Items").setValue(this.plugin.settings.customTitles.join(", ")).onChange((value) => __async(this, null, function* () {
    ...
    addButton.addEventListener("click", () => __async(this, null, function* () {
      const newRange = {
        min: this.plugin.settings.colorRanges.length > 0 ? this.plugin.settings.colorRanges[this.plugin.settings.colorRanges.length - 1].max + 1 : 1,
        max: this.plugin.settings.colorRanges.length > 0 ? this.plugin.settings.colorRanges[this.plugin.settings.colorRanges.length - 1].max + 5 : 5,
        color: "#FFE8E8"
      };
    ...
    new import_obsidian3.Setting(containerEl).setName("Default View").setDesc("Set the default time range for the heatmap display").addDropdown((dropdown) => dropdown.addOption("year", "Year View").addOption("month", "Month View").setValue(this.plugin.settings.defaultView).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.defaultView = value;
      yield this.plugin.saveSettings();
      this.plugin.refreshView();
    })));
    new import_obsidian3.Setting(containerEl).setName("Enable Cache").setDesc("Enable data caching to improve performance").addToggle((toggle) => toggle.setValue(this.plugin.settings.cacheEnabled).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian3.Setting(containerEl).setName("Refresh Data").setDesc("Manually refresh list count data").addButton((button) => button.setButtonText("Refresh").onClick(() => __async(this, null, function* () {
      yield this.plugin.refreshData();
    })));
  }
  addColorRangeRow(table, range, index) {
    ...
    const colorPicker = new import_obsidian3.ColorComponent(colorCell);
    colorPicker.setValue(range.color);
    colorPicker.onChange((value) => __async(this, null, function* () {
    ...
    minInput.addEventListener("change", () => __async(this, null, function* () {
    ...
    maxInput.addEventListener("change", () => __async(this, null, function* () {
    ...
    colorInput.addEventListener("change", () => __async(this, null, function* () {
